...
def check_cfg(cfg):
    ...
    if calc_f_mo:
        if MO_calibration_curve:
            if (not type(MO_calibration_curve) in (list, tuple)
                or (not type(MO_calibration_curve[0]) in (list, tuple))):
                print("Calibration curve must by of type 'list' or 'tuple': ",
                      MO_calibration_curve)
                return False
